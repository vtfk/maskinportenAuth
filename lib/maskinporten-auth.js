...
const generateMaskinportenGrant = options => {
  ...
  if (options.pfxcert && !options.privateKeyPassphrase) {
    throw Error('Missing required input: options.privateKeyPassphrase')
  }
  if (!options.audience) {
  ...
  if (options.pfxcert) {
    ...
    const cert = pfxToPem(options.pfxcert, options.privateKeyPassphrase)
