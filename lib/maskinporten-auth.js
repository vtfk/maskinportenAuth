 ...
 * @param {string} options.jwt The generated JWT grant from `generateMaskinportenGrant()`
 */
const getMaskinportenToken = async (options) => {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Failed to fetch token from Maskinporten. Status: ${response.status}, StatusText: ${response.statusText}. Error: ${errorText}`)
  }
